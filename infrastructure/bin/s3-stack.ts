...
const domainName = app.node.tryGetContext('domainName') || process.env.DOMAIN_NAME || 'example.com';
...
const bucketName = app.node.tryGetContext('bucketName') || process.env.BUCKET_NAME || `website-${awsAccount}-${awsRegion}`;

// Create the website bucket stack
new WebsiteBucketStack(app, `Website-${environment}`, {
